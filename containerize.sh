...
docker run -u root -it --rm -v "$(pwd)":/tmp/$bentoservice_dir -w /tmp/$bentoservice_dir buslovaev/sinara-notebook python3 save_extra_info.py --bentoservice_dir=/tmp/$bentoservice_dir --image_tag=$dockerImage
